...
class CourseCertificateTests(MockedESTestCase):
    ...
    @patch('grades.signals.generate_program_certificate_after_course_cert_created', autospec=True)
    def test_create_course_certificate(self, generate_program_cert_mock, mock_on_commit):
        ...
        """
        course_cert = MicromastersCourseCertificateFactory.create(final_grade=self.final_grade)
        generate_program_cert_mock.assert_called_with(course_cert)
