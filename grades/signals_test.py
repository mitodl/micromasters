...
class CourseCertificateTests(MockedESTestCase):
    ...
    def test_create_course_certificate(
        self, generate_program_cert_mock, mock_on_commit
    ):
        """
...
class ProgramCertificateTests(MockedESTestCase):
    ...
    def test_create_course_certificate(
        self, generate_program_letter_mock, mock_on_commit
    ):
        """
        ...
        cert = MicromastersProgramCertificate.objects.create(
            user=self.user, program=program
        )
...
class ProctoredExamGradesTests(MockedESTestCase):
    ...
    @patch(
        "exams.signals.update_existing_combined_final_grade_for_exam_run", autospec=True
    )
    def test_create_exam_grade(self, update_grade_mock, mock_on_commit):
        ...
        """
        exam_grade = ProctoredExamGradeFactory.create(
...
class FinalGradeTests(MockedESTestCase):
    ...
    def test_create_final_grade(
        self, generate_letter_mock, update_grade_mock, mock_on_commit
    ):
        """
        ...
        generate_letter_mock.assert_called_once_with(
            self.user, course_run.course.program
        )
