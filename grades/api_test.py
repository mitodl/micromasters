...
class GradeAPITests(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.enrollments = {
            course_run.edx_course_key: CachedEnrollmentFactory.create(
                user=cls.user, course_run=course_run
            )
    ...
    def test_compute_grade_for_fa(self):
        ...
        run2_data = self.user_edx_data.get_run_data(
            self.run_fa_with_cert.edx_course_key
        )
        ...
        assert grade1_from_cur_grade.passed == self.current_grades.get(
            self.run_fa.edx_course_key
        ).data.get("passed")
        assert grade1_from_cur_grade.grade == self.current_grades.get(
            self.run_fa.edx_course_key
        ).data.get("percent")
        assert grade1_from_cur_grade.payed_on_edx == (
            self.enrollments.get(self.run_fa.edx_course_key).data.get("mode")
            in ["verified", "honor"]
        )
    ...
    def test_compute_grade_for_fa_certs(self):
        """
        Tests for _compute_grade_for_fa function with CurrentGrade info
        """
        ...
        assert grade.payed_on_edx is True
    ...
    def test_compute_grade_for_non_fa(self):
        ...
        run4_data = self.user_edx_data.get_run_data(
            self.run_no_fa_with_cert.edx_course_key
        )
    ...
    @ddt.unpack
    def test_compute_grade_for_non_fa_certs(
        self, certificate_type, status, grade_result, payed_edx_result
    ):
        """
    ...
    def test_compute_grade_odd_grade(self, odd_value):
        ...
        test_values = (
            (
                self.run_fa,
                api._compute_grade_for_fa,
            ),
            (
                self.run_no_fa,
                api._compute_grade_for_non_fa,
            ),
        )
    ...
    @patch(
        "dashboard.api_edx_cache.CachedEdxDataApi.update_all_cached_grade_data",
    ...
    def test_freeze_user_final_grade_error1(
        ...
        assert (
            FinalGrade.objects.filter(
                user=self.user, course_run=self.run_no_fa
            ).exists()
            is False
        )
    ...
    @ddt.data(True, False)
    def test_freeze_user_final_grade_error1_improperly_configured(
        self, raise_on_exception, mock_refr, mock_get_fg
    ):
        """
        Test for freeze_user_final_grade function in case of problems with can_freeze_grades
        ...
        with self.assertRaises(ImproperlyConfigured):
            api.freeze_user_final_grade(
                self.user, self.run_fa_with_cert, raise_on_exception=raise_on_exception
            )
        assert mock_refr.called is False
        assert mock_get_fg.called is False
        assert (
            FinalGrade.objects.filter(
                user=self.user, course_run=self.run_fa_with_cert
            ).exists()
            is False
        )
    ...
    @ddt.data(True, False)
    def test_freeze_user_final_grade_error2(
        self, raise_on_exception, mock_refr, mock_get_fg
    ):
        """
        ...
        if not raise_on_exception:
            final_grade = api.freeze_user_final_grade(
                self.user, self.run_fa, raise_on_exception=raise_on_exception
            )
            assert final_grade is None
        else:
            with self.assertRaises(FreezeGradeFailedException):
        ...
        assert (
            FinalGrade.objects.filter(user=self.user, course_run=self.run_fa).exists()
            is False
        )
    ...
    @ddt.data(True, False)
    def test_freeze_user_final_grade_error3(
        self, raise_on_exception, mock_refr, mock_get_fg
    ):
        """
...
class GenerateCertificatesAPITests(MockedESTestCase):
    ...
    def test_successful_program_certificate_generation(self):
        ...
        with mute_signals(post_save):
            MicromastersCourseCertificate.objects.create(
                course=final_grade.course_run.course, user=self.user
            )

        cert_qset = MicromastersProgramCertificate.objects.filter(
    ...
    def test_successful_program_certificate_generation_with_electives(self):
        ...
        electives_set = ElectivesSet.objects.create(
            program=self.program, required_number=1
        )
        ...
        for run in [self.run_1, run_2]:
            ...
            ElectiveCourse.objects.create(
                course=run.course, electives_set=electives_set
            )
            with mute_signals(post_save):
                MicromastersCourseCertificate.objects.create(
                    course=final_grade.course_run.course, user=self.user
...
class GenerateProgramLetterApiTests(MockedESTestCase):
    ...
    def test_successful_program_letter_generation_with_electives(self):
        ...
        for _ in range(2):
            ...
            CourseRunGradingStatus.objects.create(
                course_run=run_elective, status="complete"
            )
            ElectiveCourse.objects.create(
                course=run_elective.course, electives_set=electives_set
            )

        letter_qset = MicromastersProgramCommendation.objects.filter(
            user=self.user, program=self.program, is_active=True
        )
    ...
    def test_already_has_program_letter(self):
        ...
        """
        MicromastersProgramCommendation.objects.create(
            user=self.user, program=self.program
        )
        # should not raise an exception
        api.generate_program_letter(self.user, self.program)
...
class UpdateCombinedFinalGradesTests(MockedESTestCase):
    ...
    def test_combined_grade_created_updated(self):
        ...
        """
        combined_grade_qset = CombinedFinalGrade.objects.filter(
    ...
    def test_update_existing_combined_final_grade_for_exam_run(
        self, update_or_create_mock
    ):
        """
        ...
        CombinedFinalGrade.objects.create(
            user=self.user, course=self.course_run.course, grade=0.7
        )
