...
def delete_duplicate_cert(apps, schema_editor):
    ...
    for course in courses:
        ...
        users_with_dup = (
            MicromastersCourseCertificate.objects.filter(course=course)
            .values("user")
            .annotate(Count("user"))
        ...
        for user in users_with_dup:
            best_grade = (
                FinalGrade.objects.filter(
                    user=user["user"], course_run__course=course, passed=True
                )
                .order_by("-grade")
                .first()
            )
            # trying to preserve the cert that is linked in the dashboard
            if MicromastersCourseCertificate.objects.filter(
...
def add_final_grade(apps, schema_editor):
    ...
    for certificate in certificates:
        certificate.delete()
    certificates = MicromastersCourseCertificate.objects.filter(
        course__program__live=True
    )

    for certificate in certificates:
        passing_final_grades = FinalGrade.objects.filter(
            user=certificate.user, course_run__course=certificate.course, passed=True
        ).order_by("-grade")
        ...
        for final_grade in passing_final_grades[1:]:
            hash = generate_hash_32(
                f"{certificate.user_id}|{final_grade.course_run_id}".encode()
            )
