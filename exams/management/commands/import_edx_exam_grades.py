        ...
        for row in reader:
            try:
                user_social_auth = UserSocialAuth.objects.get(
                    uid=row["username"], provider=BACKEND_MITX_ONLINE
                )
            except UserSocialAuth.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(
                        f"Could not find social auth for user for username {row['username']}"
                    )
                )
                continue
            ...
            try:
                ...
                raise CommandError(f'Could not find a course with number "{course_id}"')
            # should pick the latest past exam run
            now = now_in_utc()
            exam_run = (
                ExamRun.objects.filter(course=course, date_first_schedulable__lte=now)
            ...
            try:
                exam_authorization = ExamAuthorization.objects.get(
                    user=user, exam_run=exam_run
                )
            except ExamAuthorization.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(
                        "Could not find authorization for user {} and exam run {}".format(
                            user.username, exam_run.id
                        )
                    )
                )
                continue
            ...
            if int(row["no_show"]):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            "Failed to create grade: empty score for user {} and exam run {}".format(
                                user.username, exam_run.id
                            )
                        )
                    )
                ...
                _, created = ProctoredExamGrade.objects.update_or_create(
                    user=user, course=course, exam_run=exam_run, defaults=defaults
                )
                if created:
                    grade_count += 1
                    exam_authorization.exam_taken = True
