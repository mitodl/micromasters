...
class Command(BaseCommand):
    ...
    def handle(
        ...
        edx_exam_course_key = re.search(r"key:\(([A-Za-z0-9+.]+)", catalog_query).group(
            1
        )
        ...
        try:
            ...
            raise CommandError(f'Could not find a course with number "{course_number}"')
        except Course.MultipleObjectsReturned:
            raise CommandError(
                f'There are multiple courses with given number "{course_number}"'
            )
        without_timezone = datetime.strptime(
            first_row["Coupon Expiry Date"], "%b %d, %y"
        )
