...
from exams.tasks import authorize_exam_runs, authorize_enrollment_for_exam_run
...
class ExamRunTasksTest(MockedESTestCase):
    ...
    def test_authorize_exam_runs(
        self, authorized, authorize_for_latest_passed_course_mock
    ):
        ...
        past_run = ExamRunFactory.create(
            course=course, scheduling_future=True, authorized=authorized
        )
        future_run = ExamRunFactory.create(
            course=course, scheduling_past=True, authorized=authorized
        )
        authorize_exam_runs()
    ...
    @patch("exams.tasks.authorize_for_latest_passed_course")
    def test_authorize_enrollment_for_exam_run(
        self, authorize_for_latest_passed_course_mock
    ):
        ...
        authorize_enrollment_for_exam_run(
            [enrollment_1.id, enrollment_2.id], exam_run.id
        )

        assert authorize_for_latest_passed_course_mock.call_count == 2
        authorize_for_latest_passed_course_mock.assert_any_call(
            enrollment_1.user, exam_run
        )
        authorize_for_latest_passed_course_mock.assert_any_call(
