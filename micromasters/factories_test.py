...
def test_extract_related_model_kwargs():
    ...
    related_model_kwargs = extract_related_model_kwargs(
        factory_kwargs, "non_existent_model_name"
    )
    assert related_model_kwargs == {}
    assert factory_kwargs == {
        "username": "joe",
...
def test_extract_related_model_kwargs_prefix():
    ...
    assert set(related_model_kwargs.keys()) == {"username", "other_user__username"}
