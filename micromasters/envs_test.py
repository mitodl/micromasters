...
from micromasters.envs import (EnvironmentVariableParseException, get_any,
                               get_bool, get_int, get_list_of_str, get_string)
    ...
    with patch('micromasters.envs.os', environ=FAKE_ENVIRONS):
        ...
        for key, value in FAKE_ENVIRONS.items():
            if key not in ('true', 'false'):
                with pytest.raises(EnvironmentVariableParseException) as ex:
                    get_bool(key, 1234)
                assert ex.value.args[0] == f'Expected value in {key}={value} to be a boolean'
    ...
    with patch('micromasters.envs.os', environ=FAKE_ENVIRONS):
        ...
        for key, value in FAKE_ENVIRONS.items():
            if key != 'list_of_str':
                ...
                assert ex.value.args[0] == f'Expected value in {key}={value} to be a list of str'
