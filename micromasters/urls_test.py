...
class URLTests(TestCase):
    ...
    def test_urls(self):
        ...
        assert (
            reverse("profile-detail", kwargs={"user": "xyz"}) == "/api/v0/profiles/xyz/"
        )
        assert (
            reverse("profile-detail", kwargs={"user": "abc+xyz"})
            == "/api/v0/profiles/abc+xyz/"
        )
        assert (
            reverse("dashboard_api", args=["username"]) == "/api/v0/dashboard/username/"
        )
        assert (
            reverse("dashboard_api", args=["user+name"])
            == "/api/v0/dashboard/user+name/"
        )
        assert (
            reverse("search_api", kwargs={"opensearch_url": "opensearch"})
            == "/api/v0/search/opensearch"
        )
        ...
        assert (
            reverse("learner_mail_api", kwargs={"student_id": 123})
            == "/api/v0/mail/learner/123/"
        )
