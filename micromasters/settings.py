...
from micromasters.envs import get_any, get_bool, get_int, get_list_of_str, get_string
...
SECRET_KEY = get_string(
    "SECRET_KEY", "36boam8miiz0c22il@3&gputb=wrqr2plah=0#0a_bknw9(2^r"
)
...
EMAIL_BACKEND = get_string(
    "MICROMASTERS_EMAIL_BACKEND", "django.core.mail.backends.smtp.EmailBackend"
)
...
MAILGUN_BCC_TO_EMAIL = get_string(
    "MAILGUN_BCC_TO_EMAIL", "no-reply@micromasters.mit.edu"
)
...
ENROLLMENT_BY_ELIGIBILITY_COURSES = get_list_of_str(
...
WAGTAILIMAGES_MAX_UPLOAD_SIZE = get_int(
    "WAGTAILIMAGES_MAX_UPLOAD_SIZE", 20971620
)  # default 25 MB
...
AWS_DEFAULT_ACL = get_string(
    "AWS_DEFAULT_ACL", None
)  # None means use bucket's default ACL
AWS_S3_REGION_NAME = get_string("AWS_S3_REGION_NAME", None)  # e.g., 'us-east-1'
# Provide nice validation of the configuration
if MICROMASTERS_USE_S3 and (
...
CELERY_TASK_ALWAYS_EAGER = get_bool("CELERY_TASK_ALWAYS_EAGER", False) or get_bool(
    "CELERY_ALWAYS_EAGER", False
)
CELERY_TASK_EAGER_PROPAGATES = get_bool(
    "CELERY_TASK_EAGER_PROPAGATES", True
) or get_bool("CELERY_EAGER_PROPAGATES_EXCEPTIONS", True)
CELERY_BEAT_SCHEDULE = {
    ...
    'sync-course-run-info-once-a-day': {
        'task': 'courses.tasks.sync_course_run_info_from_learn',
...
UPDATE_EDX_DATA_FOR_DEDP_PROGRAM_USERS = get_bool(
    "UPDATE_EDX_DATA_FOR_DEDP_PROGRAM_USERS", False
)
...
FEATURES = {
    key[len(MM_FEATURES_PREFIX) :]: get_any(key, None)
...
MIDDLEWARE_FEATURE_FLAG_COOKIE_NAME = get_string(
    "MIDDLEWARE_FEATURE_FLAG_COOKIE_NAME", "MM_FEATURE_FLAGS"
)
...
HUBSPOT_CONFIG = {
    ...
    "HUBSPOT_PORTAL_ID": get_string(name="HUBSPOT_PORTAL_ID", default="8677455"),
}
