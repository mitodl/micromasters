...
def update_work_history(work_history_list, profile_id):
    ...
    for work_history in work_history_list:
        ...
        work_history_serializer = EmploymentSerializer(
            instance=work_history_instance, data=work_history
        )
    ...
    Employment.objects.filter(profile_id=profile_id).exclude(
        id__in=saved_work_history_ids
    ).delete()
...
def update_education(education_list, profile_id):
    ...
    for education in education_list:
        ...
        if education_id is not None:
            try:
                education_instance = Education.objects.get(
                    profile_id=profile_id, id=education_id
                )
        ...
        education_serializer = EducationSerializer(
            instance=education_instance, data=education
        )
    ...
    Education.objects.filter(profile_id=profile_id).exclude(
        id__in=saved_education_ids
    ).delete()
...
class ProfileFilledOutSerializer(ProfileSerializer):
    ...
    def validate(self, attrs):
        ...
        if (
            "agreed_to_terms_of_service" in attrs
            and not attrs["agreed_to_terms_of_service"]
        ):
