...
class EdxPipelineApiTest(MockedESTestCase):
    ...
    def test_login_redirect_dashboard(self):
        ...
        backend = edxorg.EdxOrgOAuth2(strategy=mock_strategy)
        pipeline_api.update_profile_from_edx(
            backend, student, {}, False, **{"edx_profile": self.mocked_edx_profile}
        )
    ...
    def test_login_redirect_learners(self):
        ...
        for role in [Staff.ROLE_ID, Instructor.ROLE_ID]:
            ...
            backend = edxorg.EdxOrgOAuth2(strategy=mock_strategy)
            pipeline_api.update_profile_from_edx(
                backend, user, {}, False, **{"edx_profile": self.mocked_edx_profile}
            )
...
def test_limit_one_auth_per_backend_no_user():
    ...
    assert not pipeline_api.limit_one_auth_per_backend(
        backend=None, user=None, strategy=None, uid=None
    )
...
def test_limit_one_auth_per_backend_conflicting_auth():
    ...
    mock_get_social_auth_for_user.side_effect = (
        lambda user, provider: user.social_auth.filter(provider=provider)
    )
    backend = edxorg.EdxOrgOAuth2(strategy=mock_strategy)
    assert user.social_auth.filter(provider=backend.name).count() == 1

    with pytest.raises(AuthFailed):
        pipeline_api.limit_one_auth_per_backend(
            backend=backend, user=user, strategy=mock_strategy, uid="non-matching"
        )
...
def test_limit_one_auth_per_backend_same_auth():
    ...
    assert not pipeline_api.limit_one_auth_per_backend(
        backend=backend, user=user, strategy=mock_strategy, uid=auths.first().uid
    )
