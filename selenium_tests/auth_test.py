...
def test_signin_redirect(browser, logged_in_staff, mocker, settings):
    ...
    dashboard_patch = mocker.patch(
        "dashboard.views.UserDashboard.get",
        return_value=HttpResponse(
            status=401, content=json.dumps({"error": "message"}).encode()
        ),
    )
    browser.get("/dashboard", ignore_errors=True)
    base_url = "{uri.scheme}://{uri.netloc}/".format(
        uri=urlparse(browser.driver.current_url)
    )
    assert dashboard_patch.called
    assert browser.driver.current_url == f"{base_url}signin/?next=/dashboard/"
    assert (
        browser.driver.find_element_by_css_selector("a.signup-modal-button").text
        == "CONTINUE WITH EDX"
    )
...
def test_signin_page_no_program(browser, logged_in_staff, mocker, settings, next_url):
    ...
    continue_button = browser.driver.find_element_by_css_selector(
        "a.signup-modal-button"
    )
...
@pytest.mark.parametrize(
    "has_mitxonline_run, backend, button_text",
    [
...
def test_signin_page_program(
    ...
    assert (
        continue_button.get_attribute("href") == f"{base_url}login/{backend}/{next_qs}"
    )
