...
from exams.factories import ExamAuthorizationFactory, ExamProfileFactory, ExamRunFactory
...
class DashboardStates:  # pylint: disable=too-many-locals, too-many-public-methods
    ...
    def create_exams(
        ...
        if current:
            ...
            FinalGradeFactory.create(
                ...
                grade=0.8 if edx_passed else 0.2,
                passed=True,
            )
        else:
            if edx_passed:
        ...
        exam_run = ExamRunFactory.create(
            course=course, eligibility_past=True, scheduling_past=True
        )
        ...
        LineFactory.create(order__status=Order.FULFILLED, course_key=course_run)
    ...
    def pending_enrollment(self):
        ...
        call_command(
            ...
            "staff",
            "--course-run-title",
            "Analog Learning 100 - August 2015",
        )
        run.refresh_from_db()
    ...
    def with_coupon_pending_financial_aid(self):
        ...
        self.with_coupon(Coupon.PERCENT_DISCOUNT, True, False)
        self.user.financialaid_set.update(
    ...
    def create_paid_failed_course_run(self, *, current, in_future, fuzzy):
        ...
        if in_future:
            course_run = CourseRunFactory.create(
                course=course, edx_course_key="course-in-future"
            )
            call_command(
                "alter_data",
                "set_to_offered",
        ...
        if fuzzy:
            course_run = CourseRunFactory.create(
                course=course, edx_course_key="course-fuzzy"
            )
            call_command(
                "alter_data",
                "set_to_offered",
    ...
    def create_passed_and_offered_course_run(self, grades_frozen, with_certificate):
        ...
        if grades_frozen:
            final_grade = FinalGrade.objects.filter(
                user=self.user, course_run__course=course, passed=True
            ).first()
            CourseRunGradingStatus.objects.create(
                course_run=final_grade.course_run, status="complete"
            )
            if with_certificate:
    ...
    def create_paid_but_no_enrollable_run(self, enrollable, in_future, fuzzy):
        ...
        if enrollable:
            course_run = CourseRunFactory.create(
                course=course, edx_course_key="course-enrollable"
            )
            call_command(
                "alter_data",
                "set_to_offered",
    ...
    def two_no_show_exam_attempts(self):
        ...
        course_run = CourseRunFactory.create(
            course=course, edx_course_key="course-failed"
        )
        call_command(
            "alter_data",
            "set_to_failed",
    ...
    def create_paid_enrolled_currently_with_future_run(self):
        ...
        call_command(
            ...
            "staff",
            "--course-run-key",
            course_run_current.edx_course_key,
        )
        ...
        call_command(
            ...
            "staff",
            "--course-run-key",
            course_run_future.edx_course_key,
            "--in-future",
        )
    ...
    def __iter__(self):
        ...
        yield from (
            (generate_alter_data_call(command), command.command)
            ...
            and command.command not in ["course_info", "clear_user_dashboard_data"]
        )
        ...
        for tup in itertools.product([True, False], repeat=3):
            ...
            yield (
                bind_args(
                    self.create_paid_failed_course_run,
                    current=current,
                    in_future=in_future,
                    fuzzy=fuzzy,
                ),
        ...
        yield from (
            (
                bind_args(
                    self.create_passed_and_offered_course_run, frozen, with_certificate
                ),
            ...
            for frozen, with_certificate in [
                (True, True),
                (True, False),
                (False, False),
            ]
        ...
        yield (
            self.create_passed_and_upgrade_deadline_past,
            "passed_and_missed_deadline_and_fail_in_next",
        )
        yield (self.create_failed_course_price_pending, "failed_and_pending_price")
        yield (
            self.create_audited_passed_enrolled_again_failed,
            "create_audited_passed_enrolled_again_failed",
        )
        ...
        for tup in itertools.product([True, False], repeat=3):
            ...
            yield (
                bind_args(
                    self.create_paid_but_no_enrollable_run, enrollable, in_future, fuzzy
                ),
        ...
        yield from (
            (
                bind_args(self.with_coupon, *args),
                "coupon_{amount_type}_{program}_{free}".format(
                    amount_type=args[0],
        ...
        yield (
            self.with_coupon_no_financial_aid_application,
            "with_coupon_no_financial_aid_application",
        )
        yield (
            self.with_coupon_pending_financial_aid,
            "with_coupon_pending_financial_aid",
        )
        ...
        yield (
            self.failed_run_missed_payment_can_reenroll,
            "failed_run_missed_payment_can_reenroll",
        )
        yield (
            self.create_paid_enrolled_currently_with_future_run,
            "create_paid_enrolled_currently_with_future_run",
        )
        ...
        with connection.cursor() as cur:
            ...
            if not load_from_existing_db:
                ...
                ProgramEnrollment.objects.create(
                    user=user, program=Program.objects.get(title="Analog Learning")
                )
        ...
        with django_db_blocker.unblock():
            ...
            if use_learner_page:
                for program in Program.objects.all():
                    Role.objects.create(
                        role=Staff.ROLE_ID, user=dashboard_states.user, program=program
                    )
            filename = make_filename(
                num, name, output_directory=output_directory, use_mobile=use_mobile
            )
            ...
            if not skip_screenshot:
                ...
                browser.store_api_results(
                    dashboard_states.user.username, filename=filename
                )
                if use_learner_page:
