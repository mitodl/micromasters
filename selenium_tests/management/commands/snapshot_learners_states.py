...
class LearnersStates:
    ...
    def __iter__(self):
        ...
        yield (
            self.simple_scenario("payment_status[0]=Paid&birth_location[0]=ES"),
            "spain_paid",
        )
        ...
        yield (
            self.simple_scenario("education_level[0]=hs&company_name[0]=Goldman Sachs"),
            "high_school_goldman",
        )
        yield (
            self.simple_scenario("semester[0]=2015 - Summer&q=cam"),
            "2015_cam",
        )
        yield (self.simple_scenario("education_level[0]=m"), "grad")
