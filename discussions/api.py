...
def create_discussion_user(discussion_user):
    ...
    try:
        ...
        raise DiscussionUserSyncException(
            f"Error creating discussion user for {profile.user.username}"
        ) from ex
...
def update_discussion_user(discussion_user, allow_email_optin=False):
    ...
    try:
        ...
        raise DiscussionUserSyncException(
            f"Error updating discussion user for {profile.user.username}"
        ) from ex
...
def add_channel(
    ...
    try:
        ...
        if ex.response.status_code == statuses.HTTP_409_CONFLICT:
            raise ChannelAlreadyExistsException(f"Channel {name} already exists") from ex
        raise ChannelCreationException(f"Error creating channel {name}") from ex
