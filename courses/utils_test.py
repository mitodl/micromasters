...
class CourseUtilTests(TestCase):
    ...
    def test_get_year_season_from_course_run(self):
        ...
        test_run1 = CourseRunFactory.build(
            edx_course_key="course-v1:MITx+CTL.SC0x+1T2016", start_date=fall_2016_dt
        )
        test_run2 = CourseRunFactory.build(
            edx_course_key="MITx/14.73x_1/1T2016", start_date=fall_2016_dt
        )
        test_run3 = CourseRunFactory.build(
            edx_course_key="MITx/14.73x_1", start_date=fall_2016_dt
        )
        test_run4 = CourseRunFactory.build(
            edx_course_key="invalid", start_date=fall_2016_dt
        )
        unparseable_test_run1 = CourseRunFactory.build(
            edx_course_key="invalid", start_date=None
        )
