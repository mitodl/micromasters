...
from courses.mit_learn_api import (
...
class Command(BaseCommand):
    ...
    help = (
        "Creates missing course runs and updates existing using details from MIT Learn."
    )
    ...
    def handle(
        self, *args, **kwargs
    ):  # pylint: disable=unused-argument  # noqa: C901, PLR0915
        course_id = kwargs["course_id"]
        if course_id is None:
            self.stdout.write(
                self.style.ERROR(
                    "You must provide a course_id to import all the associated course runs."
                )
            )
            return None
        self.stdout.write(
            self.style.SUCCESS(
                f"Fetching course run data from MIT Learn API for course {course_id}"
            )
        )
