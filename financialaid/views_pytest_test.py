...
def staff_client(client, program_data):
    ...
    """
    staff_user_profile = create_enrolled_profile(
        program_data.program, role=Staff.ROLE_ID
    )
...
class TestReviewViews:
    ...
    def test_not_staff(self, client, program_data, program_review_url, role):
        ...
        """
        client.force_login(
            create_enrolled_profile(program_data.program, role=role).user
        )
    ...
    def test_unavailable_financial_aid(
        self, staff_client, program_data, program_review_url
    ):
        """
        ...
        assert (
            staff_client.get(program_review_url).status_code
    ...
    def test_filter_and_sorting(
        self, staff_client, program_data, fin_aid_status, sort_param
    ):
        """
        ...
        url = self.review_url(
            program_data.program.id, status=fin_aid_status, sort_param=sort_param
        )
        ...
        assert all(
            fin_aid.status == expected_status for fin_aid in financial_aid_objects
        )
        ...
        assert list(financial_aid_objects) == sorted(
            financial_aid_objects,
            key=lambda f: f.user.last_name,
    ...
    def test_invalid_sorting_and_filter(
        self, staff_client, program_data, fin_aid_status, sort_param
    ):
        """
    ...
    def test_view_with_search(self, staff_client, program_data):
        ...
        url = self.review_url(
            program_data.program.id, status=fin_aid_status, search_param=name_query
        )
        resp = staff_client.get(url)
        assert resp.status_code == status.HTTP_200_OK
        financial_aid_objects = resp.context_data["financial_aid_objects"]
        ...
        assert all(
            name_query
            in (fin_aid.user.profile.first_name, fin_aid.user.profile.last_name)
...
class TestFinancialAidDetailViews:
    ...
    def test_props(self, program_data):
        ...
        docs_sent_url = reverse(
            "financial_aid", kwargs={"financial_aid_id": pending_fa.id}
        )
    ...
    def test_learner_can_indicate_documents_sent(self, logged_in_client, test_props):
        ...
        resp = logged_in_client.patch(
            test_props.docs_sent_url, **test_props.docs_sent_request_params
        )
        assert resp.status_code == status.HTTP_200_OK
    ...
    def test_only_change_pending_status(
        self, logged_in_client, test_props, financial_aid_status
    ):
        """
