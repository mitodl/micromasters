...
EXAMPLE_COMMANDS = [
    ...
    ExampleCommand(
        ...
        args=[
            ...
            "staff",
            "--course-run-key",
            "course-v1:MITx+Analog+Learning+100+Apr_2017",
        ],
    ),
    ExampleCommand(
...
{commands}""".format(
    commands="".join(
        [
            EXAMPLE_COMMAND_TEMPLATE.format(
                text=command.text,
...
def course_state_editor(func):
    ...
    def wrapper(*args, **kwargs):  # pylint: disable=missing-docstring
        ...
        if not course_run:
            ...
            course_run = course.courserun_set.order_by(
                "fuzzy_start_date", "-start_date"
            ).first()
...
@course_state_editor
def set_to_passed(
    user=None, course_run=None, grade=DEFAULT_GRADE, audit=False, missed_deadline=False
):
...
def set_to_enrolled(
    user=None,
    course_run=None,
    in_future=False,  # pylint: disable=too-many-arguments
    grade=None,
    audit=False,
    missed_deadline=False,
):
    ...
    enrollable_setting = (
        dict(enrollable_past=True) if missed_deadline else dict(enrollable_now=True)
    )
    if in_future:
        set_course_run_future(course_run, save=True, **enrollable_setting)
    else:
        set_course_run_current(
            course_run, upgradeable=not missed_deadline, save=True, **enrollable_setting
        )
...
def set_to_offered(
    user=None,
    course_run=None,
    now=None,  # pylint: disable=unused-argument
    in_future=False,
    fuzzy=False,
):
...
def course_info(user=None, course=None):
    ...
    for run in course.courserun_set.order_by("-start_date").all():
        ...
        if final_grade:
            run_result[
                "final_grade"
            ] = "Grade: {}, Status: {}, Passed: {}, Paid on edX: {}".format(
                final_grade.grade,
                final_grade.status,
                final_grade.passed,
                final_grade.course_run_paid_on_edx,
            )
...
def get_past_ungraded_course_run(user=None, course=None, now=None):
    ...
    for past_run in past_runs:
        if not (
            CachedCurrentGradeHandler(user).exists(past_run)
            or FinalGrade.objects.filter(user=user, course_run=past_run).exists()
        ):
            return past_run
    raise CommandError(
        f"Can't find past run that isn't already passed/failed for Course '{course.title}'"
    )
...
ADDITIONAL_PARAMS = {
    "grade": {"type": int, "help": "Course grade"},
    "in_future": {
        "type": bool,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["action"] == EXAMPLE_USAGE_COMMAND_NAME:
            ...
            course_run_finder_params = self.filter_options(
                options, CourseRunFinder.param_keys
            )
            if course_run_finder_params:
                action_params["course_run"] = CourseRunFinder.find(
                    **course_run_finder_params
                )
            ...
            if "grade" in additional_params:
                additional_params["grade"] = Decimal(
                    int(additional_params["grade"]) / 100
                )
            # Execute the action
            ...
            if isinstance(result, CourseRun):
                self.stdout.write(
                    "Course run changed: (id: {}, edx_course_key: {})\nUser: {}; Course: {}".format(
                        result.id,
