...
class SeedDBUtilityTests(MockedESTestCase):
    ...
    def test_compile_model_data_with_added_params(self):
        ...
        model_data = compile_model_data(
            User, {"email": "email1@example.com"}, username="username1"
        )
    ...
    def test_deserialize_model_data(self):
        ...
        model_obj = deserialize_model_data(
            User, {"email": "email1@example.com"}, username="username1"
        )
...
class SeedDBDeserializationTests(MockedESTestCase):
    ...
    def test_deserialize_user_data(self):
        ...
        assert (
            CachedEnrollment.objects.filter(
                user=user, course_run=new_course_run
            ).count()
            == 1
        )
    ...
    def test_deserialize_program_data_list(self):
        ...
        for course_key in ("title", "position_in_program", "description"):
            assert (
                getattr(course, course_key)
                == self.PROGRAM_DATA["courses"][0][course_key]
            )
        ...
        for i, run in enumerate(course.courserun_set.all()):
            for key in ("title", "edx_course_key"):
                assert (
                    getattr(run, key)
                    == self.PROGRAM_DATA["courses"][0]["course_runs"][i][key]
                )
    ...
    def test_deserialize_elective_set(self):
        ...
        assert (
            not elective_courses
            or len(self.ELECTIVE_DATA.get("courses"))
            == ElectiveCourse.objects.filter(electives_set=elective_set).count()
        )
