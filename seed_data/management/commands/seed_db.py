...
from courses.models import Course, CourseRun, ElectiveCourse, ElectivesSet, Program
from dashboard.models import ProgramEnrollment
from grades.models import FinalGrade, FinalGradeStatus
from micromasters.utils import first_matching_item, get_field_names, load_json_from_file
...
def deserialize_dashboard_data(user, user_data, programs):
    ...
    """
    fake_course_runs = (
        CourseRun.objects.filter(course__program__in=programs)
    ...
    deserialize_enrollment_data(
        user, social_username, fake_course_runs, enrollment_list
    )
    deserialize_grade_data(user, fake_course_runs, grade_list)


def deserialize_enrollment_data(
    user, social_username, course_runs, enrollment_data_list
):
    """
...
def deserialize_course_run_data(course, course_run_data):
    ...
    course_run = deserialize_model_data(CourseRun, course_run_data, course=course)
    return course_run
...
def deserialize_elective_course_data(elective_set, course):
    ...
    elective_course = deserialize_model_data(
        ElectiveCourse, electives_set=elective_set, course=course, data={}
    )
...
def deserialize_program_data_list(program_data_list):
    ...
    for program_data in program_data_list:
        ...
        program_data["description"] = (
            FAKE_PROGRAM_DESC_PREFIX + program_data["description"]
        )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):  # pylint: disable=too-many-locals
        ...
        if (
            len(user_data_list) == existing_fake_user_count
            and len(program_data_list) == existing_fake_program_count
        ):
