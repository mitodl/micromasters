...
dashboard_urlpatterns = [
    re_path(
        rf"^{dashboard_url.lstrip('/')}$", DashboardView.as_view(), name="ui-dashboard"
    )
...
urlpatterns = [
    ...
    path(
        "oauth_maintenance/<str:backend>/", oauth_maintenance, name="oauth_maintenance"
    ),
    re_path(
        rf"^learner/(?P<user>{USERNAME_RE_PARTIAL})?/?",
        UsersView.as_view(),
        name="ui-users",
    ),
    re_path(
        r"^certificate/course/(?P<certificate_hash>[-\w.]+)?/?",
        CourseCertificateView.as_view(),
        name="certificate",
    ),
    re_path(
        r"^certificate/program/(?P<certificate_hash>[-\w.]+)?/?",
        ProgramCertificateView.as_view(),
        name="program-certificate",
    ),
    re_path(
        r"^letter/program/(?P<letter_uuid>[-\w.]+)?/?",
        ProgramLetterView.as_view(),
        name="program_letter",
    ),
    re_path(
        r"^records/programs/(?P<enrollment_id>[\d]+)/shared/(?P<record_share_hash>[-\w.]+)?/?",
        SharedGradeRecordView.as_view(),
        name="shared_grade_records",
    ),
    re_path(
        r"^records/programs/(?P<enrollment_id>[\d]+)",
