...
describe('CustomNoHits', () => {
  ...
  it('display when no search results', () => {
    ...
    assert.equal(results, "There are no results for this search.");
  });
  ...
  it('when error in search results and query is null', () => {
    stubCommon();
    sandbox.stub(CustomNoHits.prototype, 'getQuery').returns({ 'getQueryString': () => (null) });
    sandbox.stub(CustomNoHits.prototype, 'hasHits').returns(false);
    sandbox.stub(CustomNoHits.prototype, 'getError').returns(true);
