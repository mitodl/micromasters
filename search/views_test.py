...
class SearchTests(ESTestCase, APITestCase):
    ...
    def setUpTestData(cls):
        ...
        with mute_signals(post_save):
            cls.students = [
                (ProfileFactory.create(filled_out=True)).user for _ in range(30)
            ]
        # create the programs
        ...
        for num, student in enumerate(cls.students):
            ...
            ProgramEnrollmentFactory.create(user=student, program=program)

        # create an user with a role for one program
        cls.staff = UserFactory.create()
        Role.objects.create(user=cls.staff, program=cls.program1, role=Staff.ROLE_ID)
    ...
    def test_staff_and_instructor_in_other_program_no_results(self, role, is_enrolled):
        ...
        if is_enrolled:
            ProgramEnrollmentFactory.create(user=user, program=self.program1)
        params = {"post_filter": {"term": {"program.id": self.program1.id}}}
        self.client.force_login(user)
    ...
    def test_limits_work_with_filters(self):
        ...
        Role.objects.create(user=self.staff, program=self.program3, role=Staff.ROLE_ID)
        ...
        params = {"post_filter": {"term": {"program.id": wanted_program_id}}}
    ...
    def test_query_filters(self):
        ...
        data = {
            "post_filter": {"term": {"program.id": self.program2.id}},
            "query": {
                "multi_match": {
                    "analyzer": "folding",
                    "fields": [
    ...
    def test_from(self):
        ...
        """
        data = {
            "post_filter": {"term": {"program.id": self.program1.id}},
            "from": 0,
            "size": 50,
        }
        resp = self.assert_status_code(json=data)
    ...
    def test_filled_out(self):
        ...
        program = self.program1
        filled_out_ids = list(
            ProgramEnrollment.objects.filter(program=program).values_list(
                "user_id", flat=True
            )
        )
