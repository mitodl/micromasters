...
class SearchAPITests(ESTestCase):
    ...
    def setUpTestData(cls):
        ...
        Role.objects.create(user=cls.user, program=cls.program, role=Staff.ROLE_ID)
    ...
    def test_create_search_obj_filter(self, is_advance_search_capable):
        ...
        assert (
            len(search_query_dict["query"]["bool"]["filter"]) == 2
    ...
    @ddt.unpack
    def test_create_search_obj_metadata(
        self, is_advance_search_capable, expected_index_type
    ):
        """
        ...
        with patch(
            "search.api.Search.update_from_dict", autospec=True
        ) as mock_update_from_dict:
    ...
    def test_prepare_and_execute_search(self, is_advance_search_capable):
        ...
        params = {"size": 50}
        with patch(
            "search.api.create_search_obj", autospec=True, return_value=None
        ) as mock_create_search_obj:
    ...
    def test_search_for_field(self):
        ...
        user_ids = (
            self.program.programenrollment_set.values_list("user__id", flat=True)
    ...
    def test_all_query_matching_emails(self):
        ...
        search = create_search_obj(self.user)
        user_ids = (
            self.program.programenrollment_set.values_list("user__email", flat=True)
            .exclude(user__email=self.user.email)
            .order_by("-user__id")
        )
    ...
    @patch(
        "search.signals.transaction.on_commit", side_effect=lambda callback: callback()
    )
...
class PercolateTests(ESTestCase):
    ...
    def test_adjust_search_for_percolator(self, mock_on_commit):
        ...
        original_query = {
            ...
            "size": 50,
            "sort": [{"profile.last_name": {"order": "asc"}}],
        }
    ...
    def test_percolate_failure(self, mock_on_commit):
        ...
        failure_payload = {
            "took": 1,
            "_shards": {"total": 5, "successful": 0, "failed": 5, "failures": failures},
            "total": 0,
            "matches": [],
        }
        with mute_signals(post_save):
        ...
        with self.assertRaises(PercolateException) as ex, patch(
            "search.api.get_conn",
            return_value=Mock(search=Mock(return_value=failure_payload)),
        ):
    ...
    @ddt.data(
        *product([PercolateQuery.AUTOMATIC_EMAIL_TYPE], [True, False], [True, False])
    )
    @ddt.unpack
    def test_update_percolate_memberships(
        self, source_type, is_member, query_matches, mock_on_commit
    ):
        """
        ...
        membership = PercolateQueryMembershipFactory.create(
            user=profile.user, query=query, is_member=is_member, needs_update=False
        )
    ...
    @ddt.unpack
    def test_populate_query_memberships(
        self, source_type, is_member, query_matches, mock_on_commit
    ):
        """
        Tests that existing memberships are updated where appropriate
        ...
        for profile in profiles:
            membership = PercolateQueryMembership.objects.get(
                user=profile.user, query=query
            )
    ...
    @ddt.unpack
    def test_populate_query_inactive_memberships(
        self, is_active, has_profile, mock_on_commit
    ):
        """
        ...
        with patch("search.api.get_conn") as es_mock:
            populate_query_memberships(query.id)
            assert es_mock.return_value.search.call_count == (
