...
def batch_update_user_data_subtasks_disabled(mocker, settings):
    ...
    settings.EDX_BATCH_UPDATES_ENABLED = False
    mock_log = mocker.patch("dashboard.tasks.log", autospec=True)
    mock_refresh_user_data = mocker.patch(
        "dashboard.tasks.refresh_user_data", autospec=True
    )
    batch_update_user_data_subtasks.delay(
        [1, 2, 3], (now_in_utc() + timedelta(hours=5)).timestamp()
    )
