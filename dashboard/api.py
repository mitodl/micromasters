...
from grades.models import FinalGrade, FACourseCertificate
...
def get_certificate_url(mmtrack, course):
    ...
    if best_grade:
        ...
        if mmtrack.financial_aid_available:
            fa_certificate = FACourseCertificate.objects.filter(user=mmtrack.user, course=course).first()
            if fa_certificate and course.signatories.exists():
                url = reverse('certificate', args=[fa_certificate.hash])
