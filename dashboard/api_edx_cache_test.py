...
from courses.factories import CourseFactory, CourseRunFactory, FullProgramFactory
...
class CachedEdxUserDataTests(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.current_grades = [
            CachedCurrentGradeFactory.create(user=cls.user, course_run=course_run)
            for course_run in all_course_runs
        ]
    ...
    def assert_edx_data_has_given_ids(self, edx_user_data, ids):
        ...
        assert sorted(edx_user_data.enrollments.get_enrolled_course_ids()) == sorted(
            ids
        )
        assert sorted(edx_user_data.certificates.all_courses_verified_certs) == sorted(
            ids
        )
    ...
    def test_edx_data_fetch_and_set(self):
        ...
        self.assert_edx_data_has_given_ids(
            edx_user_data, self.p1_course_run_keys + self.p2_course_run_keys
        )
...
class CachedEdxDataApiTests(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.edx_client.enrollments.get_student_enrollments.return_value = (
            cls.enrollments
        )
        cls.edx_client.certificates.get_student_certificates.return_value = (
            cls.certificates
        )
        cls.edx_client.current_grades.get_student_current_grades.return_value = (
            cls.current_grades
        )
    ...
    def assert_cache_in_db(
        self, enrollment_keys=None, certificate_keys=None, grades_keys=None
    ):
        """
        ...
        assert sorted(list(certificates.certificates.keys())) == sorted(
            certificate_keys
        )
    ...
    def test_get_cached_edx_data(self):
        ...
        self.assert_cache_in_db(
            self.all_course_run_ids, self.all_course_run_ids, self.all_course_run_ids
        )
    ...
    def test_update_cache_last_access(self):
        ...
        CachedEdxDataApi.update_cache_last_access(
            self.user, CachedEdxDataApi.ENROLLMENT, old_timestamp
        )
    ...
    def test_is_cache_fresh(self):
        ...
        models.UserCacheRefreshTime.objects.update_or_create(
            user=self.user, defaults=updated_values
        )
    ...
    def test_are_all_caches_fresh_if_social_auth_exists(self):
        ...
        user_cache, _ = models.UserCacheRefreshTime.objects.update_or_create(
            user=self.user, defaults=updated_values
        )
        assert CachedEdxDataApi.are_all_caches_fresh(self.user) is False
    ...
    def test_update_cached_enrollment(self, mocked_index):
        ...
        CachedEdxDataApi.update_cached_enrollment(
            self.user, enrollment, course_id, False
        )
        ...
        CachedEdxDataApi.update_cached_enrollment(
            self.user, enrollment_new, course_id, True
        )
        self.assert_cache_in_db(enrollment_keys=[course_id])
    ...
    def test_update_cached_enrollments(self, mocked_index):
        ...
        self.assert_cache_in_db()
        assert UserCacheRefreshTime.objects.filter(user=self.user).exists() is False
        CachedEdxDataApi.update_cached_enrollments(
            self.user, self.edx_client, BACKEND_EDX_ORG
        )
        ...
        self.assert_cache_in_db(
            enrollment_keys=list(self.enrollment_ids)
        ...
        cache_time.refresh_from_db()
        assert cache_time.enrollment >= now
        mocked_index.delay.assert_called_once_with(
            [self.user.id], check_if_changed=True
        )
    ...
    def test_update_cached_certificates(self, mocked_index):
        ...
        self.assert_cache_in_db()
        assert UserCacheRefreshTime.objects.filter(user=self.user).exists() is False
        CachedEdxDataApi.update_cached_certificates(
            self.user, self.edx_client, BACKEND_EDX_ORG
        )
        ...
        self.assert_cache_in_db(
            certificate_keys=list(self.verified_certificates_ids)
            + [cached_cert.course_run.edx_course_key]
        )
        CachedEdxDataApi.update_cached_certificates(
            self.user, self.edx_client, BACKEND_EDX_ORG
        )
    ...
    def test_update_cached_current_grades(self, mocked_index):
        ...
        self.assert_cache_in_db()
        assert UserCacheRefreshTime.objects.filter(user=self.user).exists() is False
        CachedEdxDataApi.update_cached_current_grades(
            self.user, self.edx_client, BACKEND_EDX_ORG
        )
        ...
        self.assert_cache_in_db(
            grades_keys=list(self.grades_ids) + [cached_grade.course_run.edx_course_key]
        )
        CachedEdxDataApi.update_cached_current_grades(
            self.user, self.edx_client, BACKEND_EDX_ORG
        )
    ...
    def test_update_cache_if_expired(self, provider, mock_enr, mock_cert, mock_grade):
        ...
        with self.assertRaises(ValueError):
            CachedEdxDataApi.update_cache_if_expired(
                self.user, self.edx_client, "footype", provider
            )
        ...
        for cache_type in CachedEdxDataApi.CACHE_TYPES_BACKEND[provider]:
            ...
            assert UserCacheRefreshTime.objects.filter(user=self.user).exists() is False
            CachedEdxDataApi.update_cache_if_expired(
                self.user, self.edx_client, cache_type, BACKEND_EDX_ORG
            )
        ...
        for cache_type in CachedEdxDataApi.CACHE_TYPES_BACKEND[provider]:
            CachedEdxDataApi.update_cache_if_expired(
                self.user, self.edx_client, cache_type, provider
            )
        for mock_func in all_mocks:
    ...
    def test_update_all_cached_grade_data(
        self, mock_refr, mock_enr, mock_cert, mock_grade
    ):
