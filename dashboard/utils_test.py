...
class MMTrackTest(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cached_edx_user_data = MagicMock(
            ...
            current_grades=CachedCurrentGrade.deserialize_edx_data(
                cls.current_grades_json
            ),
        )
        ...
        for course_key in expected_course_keys:
            course_run = CourseRunFactory.create(
                course=cls.course, edx_course_key=course_key
            )
            if course_key:
                cls.cruns.append(course_run)

    def pay_for_fa_course(
        self, course_id, status=None
    ...
    def test_is_enrolled(self):
        ...
        for course_id in [
            ...
            with patch(
                "edx_api.enrollments.models.Enrollments.is_enrolled_in",
                return_value=False,
            ):
    ...
    def test_has_passed_course_run(self, final_grade_passed):
        ...
        final_grade = FinalGradeFactory.create(
            user=self.user, course_run=self.cruns[0], passed=final_grade_passed
        )
        mmtrack = MMTrack(
            user=self.user,
            program=self.program,
        ...
        assert (
            mmtrack.has_passed_course_run(final_grade.course_run.edx_course_key)
            is final_grade_passed
        )
    ...
    def test_get_final_grade_percent(self):
        ...
        """
        final_grade = FinalGradeFactory.create(
            user=self.user, course_run=self.cruns[0], grade=0.57
        )
        mmtrack = MMTrack(
            user=self.user,
            program=self.program,
        ...
        assert (
            round(
                mmtrack.get_final_grade_percent(final_grade.course_run.edx_course_key)
            )
            == 57.0
        )
    ...
    def test_get_final_grade(self):
        ...
        assert (
            mmtrack.get_final_grade(final_grade.course_run.edx_course_key)
            == final_grade
        )
    ...
    def test_get_required_final_grade(self):
        ...
        assert (
            mmtrack.get_required_final_grade(final_grade.course_run.edx_course_key)
            == final_grade
        )
    ...
    def test_get_current_grade(self):
        ...
        with patch(
            "edx_api.grades.models.CurrentGradesByUser.get_current_grade",
            return_value=None,
        ):
    ...
    def test_count_passing_courses_for_keys(self):
        ...
        final_grade = FinalGradeFactory.create(
            user=self.user, course_run__course__program=self.program, passed=True
        )
        mmtrack.edx_course_keys.add(final_grade.course_run.edx_course_key)
    ...
    @ddt.unpack
    def test_has_passing_certificate(
        self, certificate_type, is_passing, expected_result
    ):
        """
        ...
        cached_edx_user_data = MagicMock(
            ...
            certificates=CachedCertificate.deserialize_edx_data(
                self.certificates_json + [cert_json]
            ),
            current_grades=CachedCurrentGrade.deserialize_edx_data(
                self.current_grades_json
            ),
        )
        mmtrack = MMTrack(
            user=self.user, program=self.program, edx_user_data=cached_edx_user_data
        )
    ...
    def test_get_best_final_grade_for_course(self):
        ...
        FinalGradeFactory.create(
            user=self.user, course_run=self.cruns[0], grade=0.3, passed=False
        )
        ...
        for grade in [0.3, 0.5, 0.8]:
            ...
            FinalGradeFactory.create(
                user=self.user, course_run=course_run, grade=grade, passed=True
            )
    ...
    def test_get_overall_final_grade_for_course(self, before_exam_merge):
        ...
        FinalGradeFactory.create(
            user=self.user, course_run=course_run, passed=True, grade=0.8
        )
    ...
    def test_get_best_proctored_exam_grade(self):
        ...
        ProctoredExamGradeFactory.create(
            user=self.user, course=course, passed=False, percentage_grade=0.6
        )
    ...
    @ddt.data(
        ...
        [
            ExamProfile.PROFILE_SUCCESS,
            ExamProfile.PROFILE_SCHEDULABLE,
            True,
            True,
            True,
        ],
    ...
    def test_get_exam_card_status_for_edx_exams(
        self, profile_status, expected_status, make_exam_run, make_profile, make_auth
    ):
        """
