...
from dashboard.factories import ProgramEnrollmentFactory, UserCacheRefreshTimeFactory
...
class DashboardTest(MockedESTestCase, APITestCase):
    ...
    def setUpTestData(cls):
        ...
        for program in [cls.program_1, cls.program_2, cls.program_no_live]:
            ProgramEnrollment.objects.create(user=cls.user, program=program)
    ...
    def test_get_dashboard(self, mock_cache_refresh):
        ...
        assert mock_cache_refresh.call_count == len(
            CachedEdxDataApi.EDX_SUPPORTED_CACHES
        )
...
class DashboardTokensTest(MockedESTestCase, APITestCase):
    ...
    @patch(
        "backends.mitxonline.MitxOnlineOAuth2.refresh_token",
    ...
    def test_refresh_token(self, mock_refresh, mock_refresh_mitxonline):
        ...
        social_auth_2 = UserSocialAuthFactory.create(
            user=self.user, provider=BACKEND_MITX_ONLINE
        )
...
class UserCourseEnrollmentTest(MockedESTestCase, APITestCase):
    ...
    def test_refresh_token_fails(self, mock_refresh):
        ...
        """
        mock_refresh.side_effect = InvalidCredentialStored(
            "foo", status.HTTP_417_EXPECTATION_FAILED
        )
    ...
    @patch(
        "edx_api.enrollments.CourseEnrollments.create_audit_student_enrollment",
        autospec=True,
    )
    @patch("backends.utils.refresh_user_token", autospec=True)
    def test_enrollment_fails(
        self, mock_refresh, mock_edx_enr
    ...
    def test_enrollment(
        ...
        cache_enr = CachedEnrollment.objects.filter(
            user=self.user, course_run__edx_course_key=self.course_id
        ).first()
        assert cache_enr is None
        user_social = UserSocialAuthFactory.create(user=self.user, provider=backend)
        CourseRun.objects.filter(edx_course_key=self.course_id).update(
            courseware_backend=backend
        )

        enr_json = {"course_details": {"course_id": self.course_id}}
        enrollment = Enrollment(enr_json)
        mock_edx_enr = (
            mock_edx_api.return_value.enrollments.create_audit_student_enrollment
        )
        mock_edx_enr.return_value = enrollment
        resp = self.client.post(self.url, {"course_id": self.course_id}, format="json")
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data == enr_json
        mock_edx_api.assert_called_once_with(
            user_social.extra_data, COURSEWARE_BACKEND_URL[backend]
        )
...
class UnEnrollProgramsTest(MockedESTestCase, APITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.client.force_login(self.user)
        self.program_enrollments = [
            ProgramEnrollmentFactory.create(user=self.user) for _ in range(2)
        ]
    ...
    def test_unenroll_all_programs(self):
        ...
        program_ids = [
            ...
            self.program_enrollments[1].program_id,
        ]
        assert ProgramEnrollment.objects.filter(user=self.user).count() == len(
            self.program_enrollments
        )

        resp = self.client.post(self.url, {"program_ids": program_ids}, format="json")

        # test no enrollment left
        assert resp.status_code == status.HTTP_200_OK
        assert (
            ProgramEnrollment.objects.filter(
                program_id__in=program_ids, user=self.user
            ).count()
            == 0
        )
    ...
    def test_unenroll_single_programs(self):
        ...
        program_enrollment_list = ProgramEnrollment.objects.filter(user=self.user)
        assert program_enrollment_list.count() == 1
        assert program_enrollment_list[0].program_id != program_ids[0]
        assert (
            program_enrollment_list[0].program_id
            == self.program_enrollments[1].program_id
        )
    ...
    def test_unenroll_when_other_user_access_api(self):
        ...
        program_ids = [self.program_enrollments[0].program_id]
        assert ProgramEnrollment.objects.filter(user=self.invalid_user).count() == 0
...
class UserExamEnrollmentTest(MockedESTestCase, APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.exam_run = ExamRunFactory.create(
            edx_exam_course_key=cls.exam_course_id, course=cls.course_run.course
        )
    ...
    def get_with_mocked_enrollments(self):
        ...
        with patch(
            ...
            return self.client.post(
                self.url, {"exam_course_id": self.exam_course_id}, format="json"
            )
    ...
    @patch(
        "edx_api.enrollments.CourseEnrollments.create_verified_student_enrollment",
        autospec=True,
    )
    @patch("edx_api.enrollments.Enrollments.get_enrolled_course_ids", autospec=True)
    @patch("backends.utils.refresh_user_token", autospec=True)
    def test_enrollment_fails(
        self, mock_refresh, mock_enrolled_ids, mock_edx_enr
    ):  # pylint: disable=unused-argument
        """
        Test error when backend raises an exception
        """
    ...
    def test_enrollment(
        ...
        cache_enr = CachedEnrollment.objects.filter(
            user=self.user, course_run__edx_course_key=self.exam_course_id
        ).first()
        assert cache_enr is None
        user_social = UserSocialAuthFactory.create(user=self.user, provider=backend)
        CourseRun.objects.filter(edx_course_key=self.exam_course_id).update(
            courseware_backend=backend
        )
        ...
        enrollment = Enrollment(enr_json)
        mock_edx_enr = (
            mock_edx_api.return_value.enrollments.create_verified_student_enrollment
        )
        mock_edx_enr.return_value = enrollment
        resp = self.get_with_mocked_enrollments()
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data == {
            "url": urljoin(
                COURSEWARE_BACKEND_URL[backend], f"/courses/{self.exam_course_id}/"
            )
        }
        mock_edx_api.assert_has_calls(
            [
                call(
                    ...
                    COURSEWARE_BACKEND_URL["mitxonline"],
                ),
