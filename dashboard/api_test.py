...
class StatusTest(MockedESTestCase):
    ...
    def test_course_run_user_status_repr(self):
        ...
        reps_str_start = (
            "<CourseRunUserStatus for course {course} status {status} at ".format(
                course=ustat.course_run.title, status=ustat.status
            )
        )
        obj_repr = repr(ustat)
    ...
    def test_course_format_conditional_fields_struct(self):
        ...
        for key in api.CourseFormatConditionalFields.ASSOCIATED_FIELDS:
            ...
            assert isinstance(
                api.CourseFormatConditionalFields.ASSOCIATED_FIELDS[key], list
            )
...
class UserProgramInfoIntegrationTest(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.program_fin_aid = FullProgramFactory.create(
            live=True, financial_aid_availability=True
        )
    ...
    def test_format(self, update_cache, mock_cache_refresh, mock_refresh_token):
        ...
        for i in range(2):
            expected = {
                ...
                "financial_aid_availability": self.expected_programs[
                    i
    ...
    def test_returns_courseruns_for_different_backends(
        self, mock_cache_refresh, mock_token_refresh
    ):  # pylint: disable=unused-argument
        """Test that user in enrolled in future run but not enrolled in current course runs"""
...
class InfoProgramTest(MockedESTestCase):
        ...
        for num in range(2):
            cls.courses.append(
                CourseFactory.create(
                    title="title course prog1 {}".format(num), program=cls.program
                )
            )
    ...
    def test_program_with_electives(self, mock_info_course):
        ...
        for num in range(3):
            course = CourseFactory.create(
                title="title course prog1 {}".format(num), program=self.program
            )
    ...
    @patch("dashboard.api.get_info_for_course", autospec=True)
    @patch(
        "financialaid.serializers.FinancialAidDashboardSerializer.serialize",
        new_callable=MagicMock,
    )
...
class ExamAttemptsTests(CourseTests):
    ...
    def test_has_to_pay_old_payment(self, num_of_taken_exams, new_purchase, result):
        ...
        Line.objects.filter(order=order).update(
            modified_at=first_date - timedelta(weeks=1)
        )
        if new_purchase:
            run = CourseRunFactory.create(course=self.course)
            order = OrderFactory.create(user=self.user, status=Order.FULFILLED)
        ...
        for _ in range(num_of_taken_exams):
            exam_auth = ExamAuthorizationFactory.create(
                user=self.user, course=self.course, exam_taken=True
            )
    ...
    def test_has_to_pay_new_payment(self, num_of_taken_exams, new_purchase, result):
        ...
        Line.objects.filter(order=order).update(
            modified_at=self.now - timedelta(weeks=3)
        )
        if new_purchase:
            run = CourseRunFactory.create(course=self.course)
            order = OrderFactory.create(user=self.user, status=Order.FULFILLED)
            ...
            # after second date
            Line.objects.filter(order=order).update(
                modified_at=self.now - timedelta(weeks=1)
            )
    ...
    @ddt.data((0, False), (1, False), (2, False), (3, True))
    @ddt.unpack
    def test_has_to_pay_old_payment_and_attempts(self, num_of_taken_exams, result):
        ...
        Line.objects.filter(order=order).update(
            modified_at=self.now - timedelta(weeks=5)
        )
        run = CourseRunFactory.create(course=self.course)
        order = OrderFactory.create(user=self.user, status=Order.FULFILLED)
        LineFactory.create(
...
class GetCertificateForCourseTests(CourseTests):
    ...
    def test_get_certificate_url(self):
        ...
        assert api.get_certificate_url(
            self.mmtrack, self.course
        ) == "/certificate/course/{}".format(cert.hash)
