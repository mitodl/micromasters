...
from grades.models import CourseRunGradingStatus, FinalGrade, ProctoredExamGrade
from grades.serializers import ProctoredExamGradeSerializer
from micromasters.factories import SocialUserFactory, UserFactory, UserSocialAuthFactory
...
class StatusTest(MockedESTestCase):
    ...
    def test_course_status_all_statuses(self):
        ...
        all_constants = [
            value
            for name, value in vars(api.CourseStatus).items()
    ...
    def test_course_format_conditional_fields_get(self):
        ...
        assert (
            len(
                api.CourseFormatConditionalFields.get_assoc_field(
...
class FormatRunTest(CourseTests):
    ...
    def setUp(self):
        ...
        self.expected_ret_data = {
            ...
            "courseware_backend": self.crun.courseware_backend,
    ...
    def test_format_run_no_run(self):
        ...
        self.assertIsNone(
            api.format_courserun_for_dashboard(
                None, api.CourseStatus.PASSED, self.mmtrack
            )
        )
    ...
    def test_format_run_normal(self):
        ...
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.PASSED, self.mmtrack
            ),
            self.expected_ret_data,
        )
    ...
    def test_format_run_different_position(self):
        ...
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.PASSED, self.mmtrack, position=56
            ),
            self.expected_ret_data,
        )
    ...
    def test_format_run_with_not_passed(self):
        ...
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.NOT_PASSED, self.mmtrack
            ),
            self.expected_ret_data,
        )
    ...
    def test_format_run_with_currently_enrolled(self):
        ...
        """
        self.expected_ret_data.update(
            {"status": api.CourseStatus.CURRENTLY_ENROLLED, "current_grade": 33.33}
        )
        del self.expected_ret_data["final_grade"]
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.CURRENTLY_ENROLLED, self.mmtrack
            ),
            self.expected_ret_data,
        )
    ...
    def test_format_run_with_can_upgrade_no_frozen_grade(self):
        ...
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.CAN_UPGRADE, self.mmtrack
            ),
            self.expected_ret_data,
        )
    ...
    def test_format_run_conditional(self):
        ...
        self.assertEqual(
            api.format_courserun_for_dashboard(
                crun, api.CourseStatus.OFFERED, self.mmtrack
            ),
            {
                "title": crun.title,
                ...
                "course_upgrade_deadline": crun.upgrade_deadline,
    ...
    def test_has_final_grade_not_enrolled(self):
        ...
        self.expected_ret_data.update(
            {"status": api.CourseStatus.MISSED_DEADLINE, "final_grade": 99.99}
        )
        self.assertEqual(
            api.format_courserun_for_dashboard(
                self.crun, api.CourseStatus.MISSED_DEADLINE, self.mmtrack
            ),
            self.expected_ret_data,
        )
...
class CourseRunTest(CourseTests):
    ...
    def test_status_with_frozen_grade(self, has_final_grades, expected_status):
        ...
        if has_final_grades:
            final_grade = FinalGradeFactory.create(
                user=self.user, course_run=self.crun, passed=True
            )
    ...
    def test_check_if_passed_if_no_frozen_grade(
        self, has_frozen_mock, freeze_grades_mock
    ):
        """
        test for get_status_for_courserun for a finished course if enrolled
    ...
    def test_not_paid_in_past(self, is_upgradable, has_final_grades, expected_status):
        ...
        with patch(
            "courses.models.CourseRun.is_upgradable", new_callable=PropertyMock
        ) as upgr_mock, patch(
            "courses.models.CourseRun.has_frozen_grades", new_callable=PropertyMock
        ) as froz_mock:
    ...
    @patch("courses.models.CourseRun.has_frozen_grades", new_callable=PropertyMock)
    @ddt.data(
        (True, api.CourseRunStatus.CAN_UPGRADE), (False, api.CourseRunStatus.NOT_PASSED)
    )
    @ddt.unpack
    ...
    @patch("courses.models.CourseRun.is_upgradable", new_callable=PropertyMock)
    @patch("courses.models.CourseRun.has_frozen_grades", new_callable=PropertyMock)
    def test_not_paid_in_past_grade_frozen_not_exists_raises(
        self, froz_mock, upgr_mock, freeze_mock
    ):
        """
        test for get_status_for_courserun for a past course run that is
        not paid, grades are already frozen, the user does not have a final grade,
    ...
    @ddt.unpack
    def test_not_paid_in_past_grade_frozen_exists(
        self, passed, status, froz_mock, upgr_mock
    ):
        """
        test for get_status_for_courserun for a past course run that is
...
class InfoCourseTest(CourseTests):
        ...
        def mock_return_status(actual_course_run, *args, **kargs):
            ...
            if actual_course_run == specific_run:
                return api.CourseRunUserStatus(
                    status=status, course_run=actual_course_run
                )
            return api.CourseRunUserStatus(
                status=other_run_status, course_run=actual_course_run
            )
    ...
    def test_info_returns_has_exam(
        ...
        exam_register_end_date = (
            f"{exam_run.date_last_schedulable.strftime('%B %-d, %I:%M %p %Z')}"
        )
    ...
    def test_info_not_enrolled_offered(
        ...
        with patch(
            ...
            return_value=api.CourseRunUserStatus(
                status=api.CourseRunStatus.NOT_ENROLLED, course_run=self.course_run
            ),
        ):
            self.assert_course_equal(
                self.course, api.get_info_for_course(self.course, self.mmtrack)
            )
        mock_format.assert_called_once_with(
            self.course_run, api.CourseStatus.OFFERED, self.mmtrack, position=1
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_not_passed_offered(
        ...
        with patch(
            ...
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.NOT_PASSED,
                self.course_run_ver,
                api.CourseRunStatus.NOT_ENROLLED,
            ),
        ):
            self.assert_course_equal(
                self.course, api.get_info_for_course(self.course, self.mmtrack)
        ...
        mock_format.assert_any_call(
            self.course_run_ver, api.CourseStatus.NOT_PASSED, self.mmtrack, position=1
        )
        # one for the course that is current run
        mock_format.assert_any_call(
            self.course_run, api.CourseStatus.OFFERED, self.mmtrack, position=2
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_not_enrolled_not_passed_not_offered(
        ...
        with patch(
            ...
            autospec=True,
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.CHECK_IF_PASSED,
                self.course_run,
                api.CourseRunStatus.CHECK_IF_PASSED,
            ),
        ...
        mock_format.assert_any_call(
            self.course_run, api.CourseStatus.NOT_PASSED, self.mmtrack, position=1
        )
        mock_format.assert_any_call(
            self.course_run_ver, api.CourseStatus.NOT_PASSED, self.mmtrack, position=2
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_grade(
        ...
        with patch(
            ...
            autospec=True,
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.CURRENTLY_ENROLLED,
                self.course_run,
                api.CourseRunStatus.CHECK_IF_PASSED,
            ),
        ):
    ...
    def test_info_missed_deadline(
        ...
        with patch(
            ...
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.NOT_ENROLLED,
                self.course_run,
                api.CourseRunStatus.MISSED_DEADLINE,
            ),
        ):
            self.assert_course_equal(
                self.course, api.get_info_for_course(self.course, self.mmtrack)
        ...
        mock_format.assert_any_call(
            self.course_run_ver,
            api.CourseStatus.MISSED_DEADLINE,
            self.mmtrack,
            position=1,
        )
        mock_format.assert_any_call(
    ...
    def test_info_will_attend(
        ...
        with patch(
            ...
            autospec=True,
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.WILL_ATTEND,
                self.course_run,
                api.CourseRunStatus.NOT_ENROLLED,
            ),
        ):
        ...
        mock_format.assert_called_once_with(
            self.course_run, api.CourseStatus.WILL_ATTEND, self.mmtrack, position=1
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_upgrade(
        ...
        with patch(
            ...
            autospec=True,
            side_effect=self.get_mock_run_status_func(
                api.CourseRunStatus.CAN_UPGRADE,
                self.course_run,
                api.CourseRunStatus.NOT_ENROLLED,
            ),
        ):
        ...
        mock_format.assert_called_once_with(
            self.course_run, api.CourseStatus.CAN_UPGRADE, self.mmtrack, position=1
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_upgrade_in_past(
        ...
        mock_format.assert_any_call(
            self.course_run_ver, api.CourseStatus.CAN_UPGRADE, self.mmtrack, position=2
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_info_default_should_not_happen(
        ...
        with patch(
            ...
            autospec=True,
            side_effect=self.get_mock_run_status_func(
                "status-that-we-should-never-have",
                self.course_run,
                api.CourseRunStatus.NOT_ENROLLED,
            ),
        ):
    ...
    def test_info_read_cert_for_all_no_next(
        ...
        mock_format.assert_any_call(
            self.course_run_past_ver, api.CourseStatus.PASSED, self.mmtrack, position=2
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_course_run_end_date_mixed(
        ...
        def mocked_get_status_for_courserun(
            run, enrollments
        ):  # pylint: disable=unused-argument
            """Mock get_status_for_courserun with different values for each run"""
            return api.CourseRunUserStatus(
                status=api.CourseRunStatus.NOT_ENROLLED, course_run=run
            )

        self.mmtrack.configure_mock(
            **{"user": self.user, "is_enrolled_mmtrack.return_value": True}
        )
        ...
        mock_format.assert_called_once_with(
            run1, api.CourseStatus.OFFERED, self.mmtrack, position=1
        )
        assert mock_schedulable.call_count == 1
        assert mock_future_exams.call_count == 1
        assert mock_get_cert.call_count == 1
    ...
    def test_course_with_proctorate_exam(
        ...
        proct_exam_qset = ProctoredExamGrade.for_user_course(
            user=self.user, course=self.course_noruns
        )
        serialized_proct_exams = ProctoredExamGradeSerializer(
            proct_exam_qset, many=True
        ).data
        self.mmtrack.get_course_proctorate_exam_results.return_value = (
            serialized_proct_exams
        )
        self.assert_course_equal(
            self.course_noruns,
            api.get_info_for_course(self.course_noruns, self.mmtrack),
        ...
        assert mock_get_cert.call_count == 1
        assert mock_exam_course_key.call_count == 1
        self.mmtrack.get_course_proctorate_exam_results.assert_called_once_with(
            self.course_noruns
        )
...
class UserProgramInfoIntegrationTest(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        for program in [
            cls.program_non_fin_aid,
            cls.program_fin_aid,
            cls.program_not_live,
        ]:
    ...
    @patch(
        "dashboard.api_edx_cache.CachedEdxDataApi.update_cache_if_expired",
    ...
    def test_format(self, update_cache, mock_cache_refresh, mock_refresh_token):
        ...
        assert mock_cache_refresh.call_count == (
            len(CachedEdxDataApi.EDX_SUPPORTED_CACHES) if update_cache else 0
        )
    ...
    def test_past_course_runs(self, mock_refresh):  # pylint: disable=unused-argument
        ...
        CourseRunGradingStatus.objects.create(
            course_run=failed_course_run, status=FinalGradeStatus.COMPLETE
        )
        ...
        CachedEnrollmentFactory.create(
            user=self.user, course_run=previous_failed_course_run
        )
        ...
        CourseRunGradingStatus.objects.create(
            course_run=previous_failed_course_run, status=FinalGradeStatus.COMPLETE
        )
        ...
        assert all(
            run["status"] == api.CourseStatus.NOT_PASSED
            for run in result["programs"][0]["courses"][0]["runs"]
        )
        assert not result["invalid_backend_credentials"]
    ...
    def test_current_run_first(self, _mock_update):  # pylint: disable=unused-argument
        ...
        assert (
            result["programs"][0]["courses"][0]["runs"][0]["status"]
            == api.CourseRunStatus.CURRENTLY_ENROLLED
        )
        assert not result["invalid_backend_credentials"]

    @patch("dashboard.api.update_cache_for_backend", autospec=True, return_value=True)
    def test_when_enroll_in_only_future_run(
        self, _mock_update
    ):  # pylint: disable=unused-argument
    ...
    @patch("dashboard.api.update_cache_for_backend", autospec=True, return_value=True)
    def test_returns_courseruns_for_different_backends(self, mock_cache_refresh):
        """Test that user in enrolled in future run but not enrolled in current course runs"""
        # create course runs for mitxonline backend
        UserSocialAuthFactory.create(user=self.user, provider=BACKEND_MITX_ONLINE)
        course_run_mitxonline = CourseRunFactory.create(
            course__program=self.program_fin_aid
        )
    ...
    @ddt.data(
        [BACKEND_EDX_ORG],
        [BACKEND_MITX_ONLINE],
        [BACKEND_MITX_ONLINE, BACKEND_EDX_ORG],
    )
    def test_returns_courseruns_for_failed_cache_update(
        self, invalid_backends, mock_cache_refresh
    ):  # pylint: disable=unused-argument
        """Test that user in enrolled in future run but not enrolled in current course runs"""
        # create course runs for mitxonline backend
        UserSocialAuthFactory.create(user=self.user, provider=BACKEND_MITX_ONLINE)
        ...
        def _raise_invalid_backend(_, provider):
            if provider in invalid_backends:
                raise InvalidCredentialStored("invalid", 400)

        mock_cache_refresh.side_effect = _raise_invalid_backend
        ...
        for program_result in result["programs"]:
            assert program_result is not None
            assert len(program_result["courses"][0]["runs"]) == 1
        assert result["invalid_backend_credentials"] == invalid_backends
...
class InfoProgramTest(MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.program_enrollment = ProgramEnrollmentFactory.create(
            program=cls.program, user=cls.user
        )
        cls.program_letter = MicromastersProgramCommendationFactory.create(
            program=cls.program, user=cls.user
        )
        ...
        for num in range(2):
            cls.courses.append(
                CourseFactory.create(
                    title=f"title course prog1 {num}", program=cls.program
                )
            )
    ...
    def test_program_with_electives(self, mock_info_course):
        ...
        electives_set = ElectivesSet.objects.create(
            program=self.program, required_number=2
        )
        for num in range(3):
            course = CourseFactory.create(
                title=f"title course prog1 {num}", program=self.program
            )
    ...
    def test_program_happy_path(self, mock_info_course):
        ...
        expected_data = {
            ...
            "has_mitxonline_courses": False,
            "has_socialauth_for_backend": True,
            "grade_records_url": reverse(
                "grade_records", args=[self.program_enrollment.id]
            ),
        }
    ...
    def test_program_for_program_letter(self, mock_info_course):
        ...
        self.mmtrack.configure_mock(
            **{
                ...
                "calculate_final_grade_average.return_value": 91,
                "get_program_certificate_url.return_value": "",
                "get_program_letter_url.return_value": reverse(
                    "program_letter", args=[self.program_letter.uuid]
                ),
...
class ExamSchedulableTests(MockedESTestCase):
    ...
    def test_is_exam_schedulable(
        self,
        is_past,
        is_future,
        has_eligibility_future,
        is_operation_delete,
        has_coupon,
        can_schedule_exam,
    ):
        ...
        exam_auth = ExamAuthorizationFactory.create(
            ...
            operation=ExamAuthorization.OPERATION_DELETE
            if is_operation_delete
            else ExamAuthorization.OPERATION_ADD,
        )
        if has_coupon:
            coupon = ExamRunCouponFactory.create(
                course=exam_run.course, coupon_url="http://example.com", is_taken=False
            )
            exam_auth.exam_coupon = coupon
            exam_auth.save()

        assert (
            api.is_exam_schedulable(exam_auth.user, exam_auth.course)
            is can_schedule_exam
        )
...
class ExamTests(MockedESTestCase):
    ...
    def test_get_edx_exam_course_key(self, auth_status, has_exam_key, returned_key):
        ...
        if has_exam_key:
            exam_run.edx_exam_course_key = exam_course_key
            exam_run.save()
    ...
    def test_is_user_enrolled_in_exam_course(self, is_enrolled, mock_enrolled_ids):
        ...
        edx_client = EdxApi(
            user_social.extra_data, COURSEWARE_BACKEND_URL[BACKEND_MITX_ONLINE]
        )
        with patch(
            ...
            assert (
                api.is_user_enrolled_in_exam_course(edx_client, exam_run) == is_enrolled
            )
...
class FutureExamRunsTests(MockedESTestCase):
    ...
    def test_get_future_exam_runs(self, is_past, is_future, result):
        ...
        exam_run = ExamRunFactory.create(
            scheduling_past=is_past, scheduling_future=is_future
        )
...
class GetCertificateForCourseTests(CourseTests):
    ...
    def setUp(self):
        ...
        self.final_grade = FinalGradeFactory.create(
            user=self.user, course_run=self.course_run, grade=0.8, passed=True
        )
    ...
    def test_get_certificate_url(self):
        ...
        cert = MicromastersCourseCertificateFactory.create(
        ...
        assert (
            api.get_certificate_url(self.mmtrack, self.course)
            == f"/certificate/course/{cert.hash}"
        )
    ...
    @ddt.unpack
    def test_edx_course_certificate(
        self, certificate_type, is_passing, has_exams, has_url
    ):
        """Test edx certificate url for non FA courses"""
        self.mmtrack.get_best_final_grade_for_course.return_value = self.final_grade
        self.mmtrack.has_exams = has_exams
        self.mmtrack.has_passing_certificate.return_value = (
        ...
        certificate_url = (
            urljoin(settings.EDXORG_CALLBACK_URL, "certificates/user/course_key")
            if has_url
            else ""
        )
...
def _make_fake_real_user():
    ...
    UserCacheRefreshTimeFactory.create(
        user=user, enrollment=now, certificate=now, current_grade=now
    )
    return user
...
def patched_redis_keys(mocker):
    ...
    mocker.patch(
        "dashboard.api.CACHE_KEY_FAILED_USERS_NOT_TO_UPDATE",
        TEST_CACHE_KEY_USER_IDS_NOT_TO_UPDATE,
    )
    mocker.patch(
        "dashboard.api.CACHE_KEY_FAILURE_NUMS_BY_USER", TEST_CACHE_KEY_FAILURES_BY_USER
    )
...
def test_calculate_fake_user(users_without_with_cache):
    ...
    assert sorted(api.calculate_users_to_refresh_in_bulk()) == sorted(
        [user.id for user in needs_update[1:]]
    )
...
@pytest.mark.parametrize(
    "enrollment,certificate,current_grade,expired",
    [
        [None, 0, 0, True],
        [0, None, 0, True],
        [0, 0, None, True],
...
def test_calculate_expired(
    users_without_with_cache, enrollment, certificate, current_grade, expired
):
    """
    ...
    cache.enrollment = (
        now_in_utc() - timedelta(hours=6, minutes=enrollment - 1)
        if enrollment is not None
        else None
    )
    cache.certificate = (
        now_in_utc() - timedelta(hours=6, minutes=certificate - 1)
        if certificate is not None
        else None
    )
    cache.current_grade = (
        now_in_utc() - timedelta(hours=6, minutes=current_grade - 1)
...
def test_refresh_user_data(db, mocker):
    ...
    edx_api = mocker.Mock()
    edx_api_init = mocker.patch(
        "dashboard.api.EdxApi", autospec=True, return_value=edx_api
    )
    update_cache_mock = mocker.patch(
        "dashboard.api.CachedEdxDataApi.update_cache_if_expired"
    )

    api.refresh_user_data(user.id, BACKEND_EDX_ORG)

    refresh_user_token_mock.assert_called_once_with(user_social)
    edx_api_init.assert_called_once_with(
        user_social.extra_data, settings.EDXORG_CALLBACK_URL
    )
...
def test_update_cache_for_backend(db, mocker, provider):
    ...
    refresh_user_token_mock.assert_called_once_with(user_social)
    edx_api_init.assert_called_once_with(
        user_social.extra_data, COURSEWARE_BACKEND_URL[provider]
    )
...
@pytest.mark.parametrize("provider", COURSEWARE_BACKENDS)
def test_update_cache_for_backend_refresh_token_error(db, mocker, provider):
    """update_cache_for_backend should raise an InvalidCredentialStored"""
    user = _make_fake_real_user()
    refresh_user_token_mock = mocker.patch(
        "dashboard.api.utils.refresh_user_token", autospec=True
    )
    refresh_user_token_mock.side_effect = InvalidCredentialStored("error", 400)
    edx_api = mocker.Mock()
    edx_api_init = mocker.patch(
        "dashboard.api.EdxApi", autospec=True, return_value=edx_api
    )
    ...
    with pytest.raises(InvalidCredentialStored):
        api.update_cache_for_backend(user, provider)

    user_social = get_social_auth(user, provider)
    refresh_user_token_mock.assert_called_once_with(user_social)
    edx_api_init.assert_not_called()
    update_cache_mock.assert_not_called()


@pytest.mark.parametrize("provider", COURSEWARE_BACKENDS)
def test_update_cache_for_backend_update_cache_error(db, mocker, provider):
    """update_cache_for_backend should raise an InvalidCredentialStored"""
    user = _make_fake_real_user()
    refresh_user_token_mock = mocker.patch(
        "dashboard.api.utils.refresh_user_token", autospec=True
    ...
    update_cache_mock.side_effect = InvalidCredentialStored("error", 400)
...
def test_refresh_failed_oauth_update(db, mocker):
    ...
    user_social = user.social_auth.first()
    refresh_user_token_mock = mocker.patch(
        "dashboard.api.utils.refresh_user_token",
        autospec=True,
...
def test_refresh_failed_edx_client(db, mocker):
    ...
    edx_api_init = mocker.patch(
        "dashboard.api.EdxApi", autospec=True, side_effect=KeyError
    )
    update_cache_mock = mocker.patch(
        "dashboard.api.CachedEdxDataApi.update_cache_if_expired"
    )
...
def test_refresh_update_cache(db, mocker, provider):
    ...
    update_cache_mock = mocker.patch(
        "dashboard.api.CachedEdxDataApi.update_cache_if_expired",
        side_effect=_update_cache,
    )
    save_failure_mock = mocker.patch(
        "dashboard.api.save_cache_update_failure", autospec=True
    )
