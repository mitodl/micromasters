...
@receiver(
    post_save, sender=ProgramEnrollment, dispatch_uid="programenrollment_post_save"
)
def handle_create_programenrollment(
...
@receiver(
    pre_delete, sender=ProgramEnrollment, dispatch_uid="programenrollment_pre_delete"
)
def handle_delete_programenrollment(
    ...
    enrollment_id = (
        instance.id
    )  # this is modified in-place on delete, so store it on a local
