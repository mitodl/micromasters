...
class UserProgramSearchSerializerTests(MockedESTestCase):
    ...
    def _generate_cached_enrollments(user, program, num_course_runs=1, data=None):
        ...
        return [
            CachedEnrollmentFactory.create(course_run=course_run, **factory_kwargs)
    ...
    def setUpTestData(cls):
        ...
        cls.enrollments = cls._generate_cached_enrollments(
            cls.user, program, num_course_runs=2
        )
        ...
        cls.serialized_course_enrollments = (
            UserProgramSearchSerializer.serialize_course_enrollments(non_fa_mmtrack)
        )
        cls.semester_enrollments = (
            UserProgramSearchSerializer.serialize_course_runs_enrolled(non_fa_mmtrack)
        )
    ...
    def test_full_program_user_serialization(self):
        ...
        serialized_enrollments = UserProgramSearchSerializer.serialize(
    ...
    def test_full_program_user_serialization_staff(self):
        ...
        self.profile.refresh_from_db()
        program = self.program_enrollment.program
        Role.objects.create(user=self.user, program=program, role=Staff.ROLE_ID)
        expected_result = {
            "id": program.id,
            "enrollments": self.serialized_enrollments,
            "courses": self.serialized_course_enrollments,
...
class UserProgramSerializerEnrollmentsTests(MockedESTestCase):
    ...
    @classmethod
    def is_course_serialized_with_status(
        cls, serialized_enrollments, course, is_verified=True
    ):
        """
        ...
        return any(
            enrollment["course_title"] == course.title
            and enrollment["payment_status"] == expected_payment_status
            for enrollment in serialized_enrollments
        )
    ...
    def all_courses_serialized(cls, serialized_enrollments, courses):
        ...
        """
        return all(
            cls.is_course_serialized(serialized_enrollments, course)
    ...
    def verified_enroll(cls, user, course_run):
        ...
        return CachedEnrollmentFactory.create(
            user=user, course_run=course_run, verified=True
        )
    ...
    def unverified_enroll(cls, user, course_run):
        ...
        return CachedEnrollmentFactory.create(
            user=user, course_run=course_run, unverified=True
        )
    ...
    def setUpTestData(cls):
        ...
        _, course_runs = cls.generate_course_with_runs(
            cls.non_fa_program, course_params={"title": "Non FA Course 1"}
        )
        cls.non_fa_enrollments = [cls.verified_enroll(cls.user, course_runs[0])]
        cls.non_fa_program_enrollment = ProgramEnrollment.objects.create(
            user=cls.user, program=cls.non_fa_program
        )
        # Create FA program data
        cls.fa_program = ProgramFactory.create()
        _, course_runs = cls.generate_course_with_runs(
            cls.fa_program, course_params={"title": "FA Course 1"}
        )
    ...
    def test_all_courses_serialized(self):
        ...
        for program_enrollment in (
            ...
            assert self.all_courses_serialized(
                serialized_enrollments, program_enrollment.program.course_set.all()
            )
    ...
    def test_other_programs_not_serialized(self):
        ...
        for program in programs_to_test:
            ...
            serialized_program_user = UserProgramSearchSerializer.serialize(
                program_enrollment_to_test
            )
            # If the new course's program is the same one we just serialized, we expect the course to be serialized.
            course_is_expected_serialized = (
                program == program_enrollment_to_test.program
            )
            course_is_serialized = self.is_course_serialized(
                serialized_program_user["courses"], course
            )
    ...
    def test_course_enrollments_serialized_unique(self):
        ...
        assert (
            len(serialized_program_user["courses"]) == serialized_count_before_addition
        )
    ...
    def test_course_final_grade_serialization(self):
        ...
        assert all(
            enrollment["final_grade"] is None
        ...
        for enrollment in self.fa_enrollments:
            FinalGradeFactory.create(
                user=self.user, course_run=enrollment.course_run, grade=0.8, passed=True
            )
        serialized_program_user = UserProgramSearchSerializer.serialize(
            self.fa_program_enrollment
        )
        assert all(
            enrollment["final_grade"] == 80.0
            for enrollment in serialized_program_user["courses"]
        )
...
class UserProgramSerializerSemesterTests(MockedESTestCase):
    ...
    def get_sorted_semester_values(serialized_program_user):
        ...
        return sorted(
            [
                serialized["semester"]
                for serialized in serialized_program_user["enrollments"]
            ],
    ...
    def test_semester_serialization(self):
        ...
        for course_run in course_runs:
            CachedEnrollmentFactory.create(
                user=self.user, course_run=course_run, verified=True
            )
        with patch(
            "dashboard.serializers.get_year_season_from_course_run",
            autospec=True,
            return_value=(2017, "Spring"),
