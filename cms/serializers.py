...
class ProgramPageSerializer(serializers.ModelSerializer):
    ...
    electives_sets = ElectivesSetSerializer(
        source="program.electives_set", many=True
    )  # Contains elective courses

    def get_courses(self, programpage):
        """Get only core courses for a program."""
        elective_course_ids = programpage.program.electives_set.filter(
            electivecourse__isnull=False
        ).values_list("electivecourse__course__id")
        ...
        return CourseSerializer(
            programpage.program.course_set.exclude(id__in=elective_course_ids),
            many=True,
        ).data
