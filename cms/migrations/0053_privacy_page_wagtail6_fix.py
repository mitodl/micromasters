...
def populate_resource_page(apps, schema_editor):
    ResourcePage = apps.get_model("cms", "ResourcePage")
    HomePage = apps.get_model("cms", "HomePage")

    # If the privacy policy page already exists (e.g., in prod/RC), do nothing
    if ResourcePage.objects.filter(slug="privacy-policy").exists():
        return

    parent_page = HomePage.objects.first()
    if not parent_page:
        # No homepage yet; nothing to attach to
        return
...
def unpopulate_resource_page(apps, schema_editor):
    """Reverse operation: delete the Privacy Policy ResourcePage created here.

    We filter by slug to avoid touching unrelated ResourcePage instances.
    """

    ResourcePage = apps.get_model("cms", "ResourcePage")
    ResourcePage.objects.filter(slug="privacy-policy").delete()


class Migration(migrations.Migration):

    dependencies = [
        ("cms", "0052_privacy_page_default_data"),
        ("wagtailcore", "0093_uploadedfile"),
        ("wagtailsearch", "0006_customise_indexentry"),
    ]

    operations = [RunPython(populate_resource_page, reverse_code=unpopulate_resource_page)]
