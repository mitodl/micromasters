...
class SearchResultMailViewsBase(MockedESTestCase, APITestCase):
    ...
    def setUpTestData(cls):
        ...
        Role.objects.create(user=cls.staff, program=cls.program, role=Staff.ROLE_ID)
    ...
    def setUp(self):
        ...
        self.recipient_tuples = [
            (context["email"], context) for context in self.email_vars
        ]
...
class SearchResultMailViewsTests(SearchResultMailViewsBase):
    ...
    def test_send_view(self):
        ...
        with patch(
            ...
            autospec=True,
            return_value=self.email_results,
            ...
            resp_post = self.client.post(
                self.search_result_mail_url, data=self.request_data, format="json"
            )
        assert resp_post.status_code == status.HTTP_200_OK
        assert resp_post.data == {}
        assert mock_get_emails.called
        assert (
            mock_get_emails.call_args[0][0].to_dict()
            == create_search_obj(
                user=self.staff,
    ...
    def test_view_response_error(self):
        ...
        """
        exception_pairs = [["b@example.com"], HTTPError()]
        with patch(
            "mail.views.get_all_query_matching_emails",
            autospec=True,
            return_value=self.email_results,
        ), patch("mail.views.MailgunClient") as mock_mailgun_client, patch(
            "mail.views.get_mail_vars",
            autospec=True,
            return_value=self.email_vars,
        ) as mock_get_mail_vars:
            mock_mailgun_client.send_batch.side_effect = SendBatchException(
                exception_pairs
            )
...
class AutomaticEmailTests(SearchResultMailViewsBase):
    ...
    def test_automatic_email(self):
        ...
        with patch(
            ...
            "mail.views.add_automatic_email",
            autospec=True,
            return_value=self.automatic_email,
        ...
        assert (
            mock_add_automatic_email.call_args[0][0].to_dict()
            == self.search_obj.to_dict()
        )
    ...
    def test_automatic_email_fail(self):
        ...
        exception_pairs = [(failure_emails, HTTPError())]
        ...
        with patch(
            ...
            "mail.views.MailgunClient",
            send_batch=Mock(side_effect=SendBatchException(exception_pairs)),
        ) as mock_mailgun_client, patch(
            "mail.views.add_automatic_email",
            autospec=True,
            return_value=self.automatic_email,
        ...
        assert sorted(
            SentAutomaticEmail.objects.filter(
                user__email__in=self.email_results,
                automatic_email=self.automatic_email,
                status=SentAutomaticEmail.SENT,
            ).values_list("user__email", flat=True)
        ) == sorted(success_emails)
...
class CourseTeamMailViewTests(APITestCase, MockedESTestCase):
    ...
    def test_course_team_email_unpaid(self):
        ...
        ProgramEnrollmentFactory.create(
            user=self.staff_user, program=new_course.program
        )
        url = reverse(self.url_name, kwargs={"course_id": new_course.id})
        resp = self.client.post(url, data=self.request_data, format="json")
        assert resp.status_code == status.HTTP_403_FORBIDDEN
...
class LearnerMailViewTests(APITestCase, MockedESTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.program = ProgramFactory.create()
        ProgramEnrollmentFactory.create(user=cls.recipient_user, program=cls.program)
        Role.objects.create(
            user=cls.staff_user, program=cls.program, role=Staff.ROLE_ID
        )
    ...
    def test_send_learner_email_view(self, mock_mailgun_client):
        ...
        url = reverse(
            self.url_name, kwargs={"student_id": self.recipient_user.profile.student_id}
        )
        resp_post = self.client.post(url, data=self.request_data, format="json")
        assert resp_post.status_code == status.HTTP_200_OK
    ...
    def test_learner_view_recipient_opted_out(self):
        ...
        with mute_signals(post_save):
            opted_out_profile = ProfileFactory.create(
                user__email="opted_out_recipient@example.com", email_optin=False
            )
        url = reverse(
            self.url_name, kwargs={"student_id": opted_out_profile.student_id}
        )
        resp_post = self.client.post(url, data=self.request_data, format="json")
        assert resp_post.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_learner_view_needs_paid_learner(self, mock_mailgun_client):
        ...
        CachedEnrollmentFactory.create(
            user=learner_user, course_run__course__program=self.program, verified=True
        )
        resp_post = self.client.post(url, data=self.request_data, format="json")
        assert resp_post.status_code == status.HTTP_200_OK
...
class EmailBouncedViewTests(APITestCase, MockedESTestCase):
    ...
    @ddt.data(
        (
            "d717895b90e49108c74df5e0cecf0d80b20e2fb2ed836bfa9209b723d57b2e77",
            status.HTTP_200_OK,
        ),
        (
            "f717895b90e49108c74df5e0cecf0d80b20e2fb2ed836bfa9209b723d57b2e88",
    ...
    @ddt.data((True, "error"), (False, "debug"))
    @ddt.unpack
    def test_bounce(self, log_error_on_bounce, logger, mock_logger):
        ...
        error_msg = "Webhook event {event} received by Mailgun for recipient {to}: {error} {header}".format(
            to=data["recipient"],
            error=data["error"],
            event=data["event"],
