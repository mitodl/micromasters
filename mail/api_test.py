...
class MailAPITests(MockedESTestCase):
    ...
    def test_send_batch(self, sender_name, mock_post):
        ...
        """
        email_body = (
            '<h1>A title</h1><p> and some text <a href="www.google.com">google</a></p>'
        )
    ...
    def test_send_batch_recipient_override(self, mock_post):
        ...
        """
        MailgunClient.send_batch(
            "subject", "body", self.batch_recipient_arg, sender_name="sender"
        )
        assert mock_post.called
        called_args, called_kwargs = mock_post.call_args
        assert list(called_args)[0] == f"{settings.MAILGUN_URL}/messages"
...
class CourseTeamMailAPITests(MockedESTestCase):
    ...
    def test_send_course_team_email(self, mock_post):
        ...
        course_with_email = CourseFactory.create(
            title="course with email", contact_email="course@example.com"
        )
    ...
    def test_send_to_course_team_without_email(self, mock_post):
        ...
        """
        course_no_email = CourseFactory.create(
            title="course no email", contact_email=None
        )
        with self.assertRaises(ImproperlyConfigured) as ex:
            MailgunClient.send_course_team_email(
                self.user, course_no_email, "email subject", "email body"
            )
...
class AutomaticEmailTests(MockedESTestCase):
    ...
    def test_send_automatic_emails(self):
        ...
        with patch(
            "mail.api.search_percolate_queries",
            autospec=True,
            return_value=self.percolate_queries,
        ...
        recipient_tuples = [
            ...
            for context in get_mail_vars([self.program_enrollment_unsent.user.email])
        ]
        mock_search_queries.assert_called_with(
            self.program_enrollment_unsent.id, PercolateQuery.AUTOMATIC_EMAIL_TYPE
        )
    ...
    def test_not_enabled(self):
        ...
        with patch(
            "mail.api.search_percolate_queries",
            autospec=True,
            return_value=[self.percolate_query_disabled],
        ) as mock_search_queries, patch("mail.api.MailgunClient") as mock_mailgun:
            send_automatic_emails(self.program_enrollment_unsent)
    ...
    def test_already_sent(self):
        ...
        mock_search_queries.assert_called_with(
            self.program_enrollment_sent.id, PercolateQuery.AUTOMATIC_EMAIL_TYPE
        )
        assert mock_mailgun.send_individual_email.called is False
    ...
    def test_failed_send(self):
        ...
        with patch(
            "mail.api.search_percolate_queries",
            autospec=True,
            return_value=[self.percolate_query, new_automatic.query],
        ) as mock_search_queries, patch(
            "mail.api.MailgunClient",
            send_individual_email=Mock(side_effect=[KeyError(), None]),
        ) as mock_mailgun:
            send_automatic_emails(self.program_enrollment_unsent)
    ...
    def test_add_automatic_email(self):
        ...
        new_automatic = add_automatic_email(
            search_obj, "subject", "body", "sender", self.staff_user
        )
        ...
        assert (
            new_automatic.query.query
            == adjust_search_for_percolator(search_obj).to_dict()
        )
    ...
    def test_mark_emails_as_sent(self, errored):
        ...
        expected = (
            [self.program_enrollment_sent.user.email] if errored else sorted(emails)
        )
        assert (
            sorted(
                self.automatic_email.sentautomaticemail_set.filter(
                    status=SentAutomaticEmail.SENT
